...
from pykms_Sql import sql_get_blacklist
...
def blacklist():
    ...
    try:
        if dbPath:
            blacklists = sql_get_blacklist(dbPath)
    except Exception as e:
        error = f'Error while loading database: {e}'

    countBlacklist = len(blacklists) if blacklists else 0
    return render_template(
        'blacklist.html',
        path='/',
        error=error,
        blacklists=blacklists,
        count_blacklist=countBlacklist
    ), 200 if error is None else 500